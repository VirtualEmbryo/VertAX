import numpy as np 

import jax.numpy as jnp 
from jax import jit, vmap
import optax 

from vertax.start import create_geograph_from_seeds
from vertax.geo import get_area, get_length, get_perimeter
from vertax.opt import inner_optax
from vertax.plot import plot_geograph

## SETTINGS

path = './_section1/results_energy_min_100cells_0.2min_dist_T1_second/'

K_areas = 20
vert_param_init = 0.   # init cond vert params 
he_param_init = 0.7    # init cond edge params
face_param_init = 0.6  # init cond area params 

min_dist_T1 = 0.2

iterations_max = 50000
tolerance = 0.00000001
patience=5

## SOLVER

sgd = optax.sgd(learning_rate=0.01)

## ENERGY TURLIER

@jit
def area_part(face: float, face_param: jnp.array, vertTable: jnp.array, heTable: jnp.array, faceTable: jnp.array):
    a = get_area(face, vertTable, heTable, faceTable)
    return (a - face_param) ** 2
@jit
def hedge_part(he: float, he_param: jnp.array, vertTable: jnp.array, heTable: jnp.array, faceTable: jnp.array):
    l = get_length(he, vertTable, heTable, L_box)
    return he_param * l
@jit
def energy_turlier(vertTable, heTable, faceTable, selected_verts, selected_hes, selected_faces, vert_params, he_params, face_params):
    mapped_areas_part = lambda face, face_param: area_part(face, face_param, vertTable, heTable, faceTable)
    mapped_hedges_part = lambda he, he_param: hedge_part(he, he_param, vertTable, heTable, faceTable)
    areas_part = vmap(mapped_areas_part)(jnp.arange(len(faceTable)), face_params)
    hedges_part = vmap(mapped_hedges_part)(jnp.arange(len(heTable)), he_params)
    return  jnp.sum(hedges_part) + (0.5 * K_areas) * jnp.sum(areas_part)

## ENERGY MINIMIZATION 

n_cells = 100
L_box = jnp.sqrt(n_cells)

seeds = L_box * np.random.random_sample((n_cells, 2))
vertTable_init, heTable_init, faceTable_init = create_geograph_from_seeds(seeds, path, show=False)
selected_verts, selected_hes, selected_faces = jnp.arange(len(vertTable_init)), jnp.arange(len(heTable_init)), jnp.arange(len(faceTable_init))

# Plot initial non-equilibrium cofiguration 
plot_geograph(vertTable_init, 
              heTable_init, 
              faceTable_init, 
              L_box=L_box, 
              multicolor=True, 
              lines=True, 
              vertices=False, 
              path=path, 
              name='intial_configuration', 
              save=True, 
              show=False)

vert_params = []
for vert in range(len(vertTable_init)):
    vert_params.append(vert_param_init)
vert_params = jnp.asarray(vert_params)

he_params = []
for hedge in range(len(heTable_init)):
    he_params.append(he_param_init)
he_params = jnp.asarray(he_params)

face_params = [] 
for face in range(len(faceTable_init)):
    face_params.append(face_param_init)
face_params = jnp.asarray(face_params)

(vertTable_eq_turlier, heTable_eq_turlier, faceTable_eq_turlier), L_in_turlier = inner_optax(vertTable_init,
                                                                                             heTable_init, 
                                                                                             faceTable_init,
                                                                                             selected_verts,
                                                                                             selected_hes,
                                                                                             selected_faces,  
                                                                                             vert_params,
                                                                                             he_params,
                                                                                             face_params,
                                                                                             energy_turlier, 
                                                                                             sgd,  
                                                                                             min_dist_T1,
                                                                                             iterations_max,
                                                                                             tolerance,
                                                                                             patience)

# Plot final equilibrium cofiguration 
plot_geograph(vertTable_eq_turlier, 
              heTable_eq_turlier, 
              faceTable_eq_turlier, 
              L_box=L_box, 
              multicolor=True, 
              lines=True, 
              vertices=False, 
              path=path, 
              name='turlier_energy_0.7he_0.6f', 
              save=True, 
              show=False)

## SETTINGS 

vert_param_init = 0.   # init cond vert params 
he_param_init = 0.    # init cond edge params
face_param_init = 3.72  # init cond area params 

vert_params = [vert_param_init]
vert_params = jnp.asarray(vert_params)

he_params = [he_param_init]
he_params = jnp.asarray(he_params)

face_params = [face_param_init] 
face_params = jnp.asarray(face_params)

## ENERGY MANNING

@jit
def cell_energy(face, face_param, vertTable, heTable, faceTable):
    area = get_area(face, vertTable, heTable, faceTable)
    perimeter = get_perimeter(face, vertTable, heTable, faceTable)
    return ((area - 1) ** 2) + ((perimeter - face_param) ** 2)

@jit
def energy_manning(vertTable, heTable, faceTable, selected_verts, selected_hes, selected_faces, vert_params, he_params, face_params):
    mapped_fn = lambda face, param: cell_energy(face, param, vertTable, heTable, faceTable)
    face_params_broadcasted = jnp.broadcast_to(face_params, (len(faceTable),) + face_params.shape[1:])
    cell_energies = vmap(mapped_fn)(jnp.arange(len(faceTable)), face_params_broadcasted)
    return jnp.sum(cell_energies)

(vertTable_eq_manning, heTable_eq_manning, faceTable_eq_manning), L_in_manning = inner_optax(vertTable_init,
                                                                                             heTable_init, 
                                                                                             faceTable_init,
                                                                                             selected_verts,
                                                                                             selected_hes,
                                                                                             selected_faces,  
                                                                                             vert_params,
                                                                                             he_params,
                                                                                             face_params,
                                                                                             energy_manning, 
                                                                                             sgd,  
                                                                                             min_dist_T1,
                                                                                             iterations_max,
                                                                                             tolerance,
                                                                                             patience)

# Plot final equilibrium cofiguration 
plot_geograph(vertTable_eq_manning, 
              heTable_eq_manning, 
              faceTable_eq_manning, 
              L_box=L_box, 
              multicolor=True, 
              lines=True, 
              vertices=False, 
              path=path, 
              name='manning_min_energy_3.72f', 
              save=True, 
              show=False)
